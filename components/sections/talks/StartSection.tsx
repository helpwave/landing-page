...
const StartSection = ({ overwriteTranslation }: PropsForTranslation<TalksTranslation>) => {
  ...
  return (
    <SectionBase>
      ...
      <span className="font-sans text-2xl font-medium mt-2 text-description">
        {translation('fusionOfHealthcareAndComputerScience')}
      </span>

      <Image
        alt="Screenshots" src={screenshotURL}
        style={{ objectFit: 'contain', width: `${size}px` }}
        width={size} height={size}
        className="shadow-md hover:shadow-2xl transition-all duration-500 w-full rounded-md mt-8"
      />
    </SectionBase>
  )
}
